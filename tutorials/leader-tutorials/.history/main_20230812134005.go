...
func startLeaderElection(context context.Context, id string, clientset *kubernetes.Clientset, stop chan struct{}) {
	...
	// 启动选主操作
	leaderelection.RunOrDie(context, leaderelection.LeaderElectionConfig{
		Lock:            lock,
...
func StartLeaderElecting(ctx context.Context, clientset *kubernetes.Clientset, stop chan struct{}) {
	lock := &resourcelock.LeaseLock{
		LeaseMeta: metav1.ObjectMeta{
			Name:      "leader-tutorials",
			Namespace: NAMESPACE,
	...
	var f = func(ctx context.Context) {
		klog.Infof("Leader election success [%s]", processIdentity)
		// 就像抢分布式锁一样，当前进程选举成功的时候，这的代码就会被执行，
		// 所以，在这里填写抢锁成功的业务逻辑吧，本例中就是监听service变化，然后修改pod的label
		CreateAndStartController(clientset.CoreV1().RESTClient(), &v1.Service{}, "services", NAMESPACE, stop)
	}
	...
	leaderelection.RunOrDie(context.TODO(), leaderelection.LeaderElectionConfig{
		...
		Callbacks: leaderelection.LeaderCallbacks{
			...
			OnStoppedLeading: func() {
				// 失去了leader时的逻辑
				klog.Infof("leader lost: %s", processIdentity)
				os.Exit(0)
			},
			OnNewLeader: func(identity string) {
				// 收到通知，知道最终的选举结果
